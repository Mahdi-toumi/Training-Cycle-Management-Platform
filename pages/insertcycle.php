...
if (isset($_SESSION['admin'])) {
    ...
    if (strtotime($date_deb) >= strtotime($date_fin)) {
        ...
        header('Location: nouveaucycle.php#date_deb');
        exit();
    }
    ...
    if (strtotime($heure_deb) >= strtotime($heure_fin)) {
        ...
        header('Location: nouveaucycle.php#heure_deb');
        exit();
    }

    if (strtotime($pause_deb) <= strtotime($heure_deb) || strtotime($pause_fin) >= strtotime($heure_fin)) {
        $_SESSION['erreurepause'] = "<strong>Erreur</strong> La pause doit être entre l'heure de début et l'heure de fin. !!!";
        header('Location: nouveaucycle.php#pause_deb');
        exit();
    }
